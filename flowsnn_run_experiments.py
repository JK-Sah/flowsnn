...
def evaluate(model, X, Y, R, is_snn=False):
    ...
    # heading MAE [degrees] — wrapped angular difference to handle ±180° boundary
    true_heading = np.degrees(np.arctan2(R[:, 1], R[:, 0]))
    pred_heading = np.degrees(np.arctan2(reg[:, 1], reg[:, 0]))
    diff = np.arctan2(
        np.sin(np.radians(pred_heading - true_heading)),
        np.cos(np.radians(pred_heading - true_heading))
    )
    hmae = float(np.degrees(np.abs(diff)).mean())
    # speed MAE [mm/s]
